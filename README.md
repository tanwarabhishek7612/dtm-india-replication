...
**Nine of the ten have been run in Stata and their output confirmed against
the manuscript's published results; the tenth has a pending re-run — see
`VERIFICATION_STATUS.md` for the full, honest record**, including two bugs
(a double-logged variable, and a missed non-adjacent tie in the CBR
reversal check) that were caught and fixed, and one flagged near-boundary
discrepancy that's worth a manual look. Each `.do` file's header documents
its own verification status, and each ends with an "EXPECTED OUTPUT" block
for anyone who wants to re-check it themselves.
