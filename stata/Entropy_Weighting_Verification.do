* Entropy-Weighting Robustness Check (Stata translation)
...
* ---------------------------------------------------------------------------
* STEP 1: min-max normalise each indicator, then clip at 1e-9 (matching
* the Python original's np.clip) so a state with the literal minimum
* value doesn't create a 0 that blows up the log() in Step 3.
* ---------------------------------------------------------------------------
